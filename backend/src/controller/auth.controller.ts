...
export const Register = async (c: Context) => {
  try {
    ...
    return c.json(
      JsonResponse(true, "User created!", { id: newUser.id }),
      201
    );
  } catch (e) {
    console.log(e);
...
export const Login = async (c: Context) => {
    try {
        ...
        if (!body.username || !body.password) return c.json(JsonResponse(false, "Missing Required field"), 400)
        ...
        if (!isPasswordMatch) return c.json(JsonResponse(false, "Incorrect Password"), 400)
